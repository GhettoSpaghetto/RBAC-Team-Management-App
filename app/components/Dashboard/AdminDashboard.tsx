        ...
        const handleTeamAssignment = async(userId: string, teamId: string | "") => {
            startTransition(async() => {
                try {
                    ...
                    alert(error instanceof Error ? error.message : "Error updating team assigment")
                }
            })
        }

        const handleRoleAssigment = async (userId: string, newRole: Role) => {
        ...
        return <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg">
                    <div className="p-4 border-b bordder-slate-700">
                        <h3 className="font-semibold text-white">Users ({users.length})</h3>
                        <p className="text-slate-400 text-sm">Manage roles and team assigment</p>
                    </div>
                    <div className="p-4">
                                ...
                                {users.map((user) => (
                                    <tr key ={user.id} className="border-b border-slate-700">
                                        ...
                                        <td className="py-2">
                                            <select 
                                            value={user.role} 
                                            onChange={(e) => handleRoleAssigment(user.id, e.target.value as Role)}
                                        ...
                                        <td className="py-2">
                                            ...
                                            <div className="flex items-center space-x-2">
                                                <select
                                                    value={user.teamId || ""}
                                                    onChange={(e) => handleTeamAssignment(user.id, e.target.value)}
                                                    disabled={isPending}
                                                    className="bg-slate-900 border border-slate-700 rounded"
                                                    >
                ...
                {/* Teams table with role and team assignment */}
                                                    ...
                                                    <div className="flex flex-wrap gap-1">
                                                        {
                                                            teamManagers.map((manager) =>(
                                                                <span key={manager.id} className="bg-blue-500/20 text-blue-300 px-2 py-1 rounded text-xs" title={manager.name}>{manager.name}</span>
                                                            ))
                                                        }
                                                    </div>
                                                ): (
            ...
            <div className="grid  grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 text-center">
                    ...
                    <div className="text-sm text-slate ">Teams</div>
                </div>
            </div>
        </div>
    }
